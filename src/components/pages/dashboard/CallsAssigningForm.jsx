...
const API = "http://localhost:5000/api/v1/calls-assigning";
...
const CallsAssigningForm = () => {
  ...
  const fetchMasters = async () => {
    try {
      ...
      await Promise.all([
        fetchAndSet("http://localhost:5000/api/v1/call-entry-form", setCalls),
        fetchAndSet("http://localhost:5000/api/v1/department", setDepartments),
        fetchAndSet("http://localhost:5000/api/v1/call-master", setCallTypes),
        (async () => {
          const res = await axios.get("http://localhost:5000/api/v1/call-nature-master");
          const data = Array.isArray(res.data)
            ? res.data
            : Array.isArray(res.data.data)
              ? res.data.data
        ...
        fetchAndSet("http://localhost:5000/api/v1/instrument-master", setInstruments),
        fetchAndSet("http://localhost:5000/api/v1/problem", setProblems),
        fetchAndSet("http://localhost:5000/api/v1/employee-master", setEmployees),
        // fetchAndSet("http://localhost:5000/api/v1/status-master", setStatuses), // Removed status
        fetchAndSet("http://localhost:5000/api/v1/call-urgency", setPriorities),
        fetchAndSet("http://localhost:5000/api/v1/customer-master", setCustomers),
        fetchAndSet("http://localhost:5000/api/v1/end-user-master", setEndUsers),
        fetchAndSet("http://localhost:5000/api/v1/designation-master", setDesignations),
        // Fetch technician names for "Assigned By" from Technician Master
        (async () => {
          const res = await axios.get("http://localhost:5000/api/v1/technician-master");
  ...
  const fetchCallEntryFormDetails = async (callNoId) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/v1/call-entry-form/${callNoId}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        // Remove "assignedBy" if empty, but if provided, convert to technician _id
        if (
          formData[key] !== "" &&
          formData[key] !== null &&
          formData[key] !== undefined
        ) {
          // For assignedBy: convert technician name to _id if possible
          if (key === "assignedBy") {
            const assignedByName = formData.assignedBy;
            if (assignedByName) {
              // Find match in technicianNames by technicianName or name (case sensitive)
              const tech = technicianNames.find(
                (t) =>
                  t.technicianName === assignedByName ||
                  t.name === assignedByName
              );
              if (tech && tech._id) {
                submitData.append("assignedBy", tech._id);
              } else {
                // fallback: still append as string, but backend will likely reject, so encourage matching
                submitData.append("assignedBy", assignedByName);
              }
            }
          } else {
  ...
  const handleEdit = (item) => {
    ...
    setFormData({
      ...
      assignedBy: (() => {
        // Try to find technician by id and get technicianName or name for display in edit form
        if (!item.assignedBy) return "";
        // Try to find in technicianNames list (either by _id or name)
        if (typeof item.assignedBy === "object" && item.assignedBy._id) {
          // Try to use technicianName or name property
          return item.assignedBy.technicianName || item.assignedBy.name || item.assignedBy._id;
        }
        // If assignedBy is likely an ObjectId or string id, find in technicianNames
        if (typeof item.assignedBy === "string") {
          const tech = technicianNames.find(
            tech =>
              tech._id === item.assignedBy ||
              tech.technicianName === item.assignedBy || 
              tech.name === item.assignedBy
          );
          return tech ? (tech.technicianName || tech.name || tech._id) : item.assignedBy;
        }
